...
const generateSparkline = (length: number, base: number, variance: number): number[] =>
...
export const getBranchData = (): BranchData[] => {
    const branches = [
        { name: 'Amman Central', nameAr: 'عمّان المركزي', region: 'Amman', regionAr: 'عمّان' },
        { name: 'Irbid Main', nameAr: 'إربد الرئيسي', region: 'North', regionAr: 'الشمال' },
        { name: 'Zarqa Branch', nameAr: 'فرع الزرقاء', region: 'Central', regionAr: 'الوسط' },
        { name: 'Aqaba Port', nameAr: 'العقبة الميناء', region: 'South', regionAr: 'الجنوب' },
        { name: 'Madaba City', nameAr: 'مادبا المدينة', region: 'Central', regionAr: 'الوسط' },
        { name: 'Salt Downtown', nameAr: 'السلط وسط البلد', region: 'Central', regionAr: 'الوسط' },
        { name: 'Karak Branch', nameAr: 'فرع الكرك', region: 'South', regionAr: 'الجنوب' },
        { name: 'Mafraq North', nameAr: 'المفرق الشمالي', region: 'North', regionAr: 'الشمال' },
        { name: 'Jerash Heritage', nameAr: 'جرش التراث', region: 'North', regionAr: 'الشمال' },
        { name: 'Tafilah South', nameAr: 'الطفيلة الجنوبي', region: 'South', regionAr: 'الجنوب' },
    ];
...
export const getProductData = (): ProductData[] => {
    const products = [
        { name: 'Premium Rice 5kg', nameAr: 'أرز فاخر 5 كجم', category: 'Groceries', categoryAr: 'بقالة', subcategory: 'Rice & Grains' },
        { name: 'Olive Oil 1L', nameAr: 'زيت زيتون 1 لتر', category: 'Groceries', categoryAr: 'بقالة', subcategory: 'Oils' },
        { name: 'Fresh Chicken 1kg', nameAr: 'دجاج طازج 1 كجم', category: 'Meat', categoryAr: 'لحوم', subcategory: 'Poultry' },
        { name: 'Detergent 3L', nameAr: 'منظف 3 لتر', category: 'Household', categoryAr: 'منزلية', subcategory: 'Cleaning' },
        { name: 'Baby Diapers Pack', nameAr: 'حفاضات أطفال', category: 'Baby Care', categoryAr: 'رعاية الأطفال', subcategory: 'Diapers' },
        { name: 'Mineral Water 6pk', nameAr: 'مياه معدنية 6 عبوات', category: 'Beverages', categoryAr: 'مشروبات', subcategory: 'Water' },
        { name: 'Canned Tuna', nameAr: 'تونة معلبة', category: 'Canned', categoryAr: 'معلبات', subcategory: 'Fish' },
        { name: 'Fresh Milk 1L', nameAr: 'حليب طازج 1 لتر', category: 'Dairy', categoryAr: 'ألبان', subcategory: 'Milk' },
        { name: 'White Sugar 1kg', nameAr: 'سكر أبيض 1 كجم', category: 'Groceries', categoryAr: 'بقالة', subcategory: 'Sugar' },
        { name: 'Sunflower Oil 2L', nameAr: 'زيت عباد الشمس 2 لتر', category: 'Groceries', categoryAr: 'بقالة', subcategory: 'Oils' },
    ];
...
export const getForecastData = (): ForecastData[] => {
    ...
    months.forEach((m, i) => {
        const base = 2000000 + i * 100000 + Math.sin(i / 2) * 300000;
        data.push({
            date: `${m} 2025`,
            actual: Math.round(base + (Math.random() - 0.5) * 400000),
    ...
    forecastMonths.forEach((m, i) => {
        const base = 2800000 + i * 120000;
        data.push({
            date: `${m} 2026`,
            actual: null,
            predicted: Math.round(base),
            upperBound: Math.round(base * 1.2),
            lowerBound: Math.round(base * 0.8),
            confidence: Number((82 - i * 2 + Math.random() * 3).toFixed(1)),
        });
    });
...
export const getCategoryDistribution = () => [
    ...
    { name: 'Meat & Poultry', nameAr: 'لحوم ودواجن', value: 22, color: '#00d4ff' },
...
export const getRegionalData = () => [
    { region: 'Amman', regionAr: 'عمّان', revenue: 9500000, branches: 15, growth: 12.5 },
    { region: 'North', regionAr: 'الشمال', revenue: 5200000, branches: 12, growth: 8.3 },
    { region: 'Central', regionAr: 'الوسط', revenue: 4800000, branches: 10, growth: 6.7 },
    { region: 'South', regionAr: 'الجنوب', revenue: 3100000, branches: 8, growth: 4.2 },
    { region: 'East', regionAr: 'الشرق', revenue: 1987300, branches: 2, growth: -1.3 },
];
