...
export type BranchProductProd = { name: string; vol: number; price: number; basket: number; atv: number };
export type BranchProductCat = { name: string; vol: number; price: number; basket: number; atv: number; products: BranchProductProd[] };
export type BranchProductAnalysisBranch = {
    branch: string;
    vol: number;
    price: number;
    basket: number;
    ...
    for (const b of analysis) {
        ...
        if (bo) {
            for (const c of b.cats) {
                const ck = `${keyPrefix}_${b.branch}_${c.name}`;
                const co = !!expandedCats[ck];
                t.push({
                    key: ck,
                    label: c.name,
                    depth: 1,
